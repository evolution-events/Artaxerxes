...
class TestPaymentService(MockMollieMixin, TestCase):
    ...
    def test_starting_payment(self):
        ...
        self.assertEquals(mollie_payment.redirect_url, self.request.build_absolute_uri(next_url))
        self.assertEquals(mollie_payment.webhook_url, self.request.build_absolute_uri(webhook_url))
        self.assertEquals(mollie_payment.checkout_url, checkout_url)
